...
public interface BoardRepository extends JpaRepository<BoardEntity, Long> {
    ...
    // Page<BoardEntity>
    // findByMemberGroupAndNotReportedAndTitleContaining(@Param("userGroup") String
    // userGroup,
    // @Param("searchWord") String searchWord, Pageable pageRequest);

    // group 게시판 목록 (반환 타입 : BoardListDTO)
    // @Query("SELECT new
    // net.dima_community.CommunityProject.dto.board.combine.BoardListDTO(" +
    // "b.boardId, " +
    ...
    // Page<BoardListDTO>
    // findBoardListByMemberGroupAndTitleContaining(@Param("userGroup") String
    // userGroup,
    ...
    // "LOWER(b.title) LIKE LOWER(CONCAT('%', :searchWord, '%'))")
    // Page<BoardEntity>
    // findByCategoryTitleContainingAndReportedIsFalse(@Param("category")
    ...
    // "ORDER BY b.createDate DESC")
    // Page<BoardListDTO>
    // findBoardListByCategoryAndTitleContaining(@Param("category") BoardCategory
    // category,
    // @Param("searchWord") String searchWord,
    // Pageable pageRequest);
    ...
    // @Modifying
    // @Query("UPDATE BoardEntity b SET b.likeCount = b.likeCount + 1 WHERE
    // b.boardId = :boardId")
    // void incrementLikeCount(Long boardId);

    // // boardId에 해당하는 Entity의 likeCount를 1 감소시킴 (연산 시 likeCount가 0보다 작은 경우는 0으로
    // 세팅)
    // @Modifying
    // @Query("UPDATE BoardEntity b " +
    // "SET b.likeCount = CASE WHEN b.likeCount - 1 < 0 THEN 0 ELSE b.likeCount - 1
    // END " +
